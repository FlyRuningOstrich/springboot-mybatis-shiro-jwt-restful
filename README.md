...
│      │      │  │      DefaultHeaderSessionManager.java    //自定义sessionid存取方法。默认cookie中存取，改为header中
│      │      │  │      HeaderRememberMeManager.java    //自定义rememberMe存取方法。默认cookie中存取，改为header中存取
│      │      │  │      MyRealm.java    //自定义认证方式。实现了给当前用户赋予权限，角色。进行验证
│      │      │  │      RedisSessionDao.java    //session的reddis支持
│      │      │  │      ShiroConfig.java    //shiro配置。注入自定义方法，增加注解支持等
│      │      │  │      ShiroLogin.java    //登入shiro方法
│      │      │  │      ShiroSessionTool.java    //通过sessionId获取shiro存储的Session对象工具
│      │      │  │
